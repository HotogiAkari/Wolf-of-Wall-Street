...
def run_training_for_ticker(
    ...
    """
    为单个模型执行完整的滚动训练、最终模型生成和版本管理。
    - 智能处理四种模式：强制重训、断点续训、追加训练、检查更新。
    - 避免在滚动训练和最终模型都已是最新时进行任何不必要的操作。
    """
    ...
    start_fold_idx = 0
    run_final_training = False # 决策是否运行最终模型训练的总开关
    ...
    # 模式二: 优先处理中断的任务
        ...
        try:
            ...
            print(f"WARNNING: 读取进度文件失败: {e}。将从头开始。")
            start_fold_idx = 0
            run_final_training = True
        ...
        try:
            ...
            if num_completed >= total_new_folds:
                ...
                if final_model_path.exists():
                    print(f"INFO: 已有的滚动训练结果 ({num_completed} folds) 和最终模型 (版本 {version_date}) 均已是最新。")
                    print(f"--- {model_type.upper()} 模型无需操作，跳过。 ---")
                    # 直接返回，什么都不做
                    return pd.read_csv(ic_history_path) if ic_history_path.exists() else None
                else:
                    print(f"INFO: 已有的训练结果 ({num_completed} folds) 已是最新，但缺少最终模型。将只训练最终模型。")
            ...
            print(f"WARNNING: 读取历史 IC 文件以确定断点时失败: {e}。将从头开始。")
            start_fold_idx = 0
            run_final_training = True
    ...
    # --- 3. 滚动训练 ---
    if start_fold_idx < (len(preprocessed_folds) or 0):
        ...
        if start_fold_idx == 0 and not progress_file.exists():
             with open(progress_file, 'w') as f:
                 json.dump({'completed_folds': 0, 'status': 'in_progress'}, f)
        ...
        for i, fold_data in fold_iterator:
            ...
            with open(progress_file, 'w') as f:
                json.dump({'completed_folds': i + 1, 'status': 'in_progress'}, f)
    ...
    if run_final_training:
        ...
        if full_df is not None:
            ...
            # 使用之前计算好的 version_date 来保存模型
            model_file = final_model_path # 使用之前定义好的路径
            ...
            if len(all_model_files) > num_to_keep:
                ...
                print(f"INFO: 清理 {len(files_to_delete)} 个旧模型版本...")
                for old_model_file in files_to_delete:
                    try:
                        ...
                        print(f"WARNNING: 清理旧版本 {old_model_file.name} 时出错: {e}")
        else:
             print("WARNNING: 未提供 full_df_for_final_model，无法训练最终模型。")
