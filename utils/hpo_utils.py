...
def objective(trial, preprocessed_folds: list, config: dict, model_type: str = 'lgbm'):
    ...
    try:
        ...
        BuilderClass = builder_registry.get(model_type)
    ...
    # 避免除以零
    if std_ic < 1e-8:
        return mean_ic * np.sign(mean_ic) * 10
...
def run_hpo_for_ticker(preprocessed_folds: list, ticker: str, config: dict, model_type: str = 'lgbm') -> tuple:
    """
    运行 HPO，使用 TQDM 回调函数来提供简洁、动态的进度反馈。
    """
    ...
    # (核心修改) 从 'data' 组获取股票池信息
    stocks_to_process = config.get('data', {}).get('stocks_to_process', [])
    keyword = next((s.get('keyword', ticker) for s in stocks_to_process if s['ticker'] == ticker), ticker)
    ...
    PARAM_MAP_CN = {
        ...
        'dim': 'Embedding维度', 'depth': 'Transformer层数', 'heads': '多头注意力头数'   # 为 TabT 预留
    }
